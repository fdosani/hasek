...
Set the username and password using the config module:
    ...
    hasek config --set connections.db1.host <host>
    hasek config --set connections.db1.username <username>
    hasek config --set connections.db1.password <password>"""
...
class Config(object):
    """
    An object for reading, writing, and encrypting hasek configuration
    ...
    :raises `hasek.errors.KeyNotFound`: if default key_file is not
    ...
    def __init__(self, conf=None, mode="r", key_file=None):
        ...
        if key_file is None:
            self._key_file = home_file(".hasekpg")
    ...
    def get_connection(self, dsn=None):
        ...
        :raises `hasek.errors.ConfigurationError`: if :code:`dsn` does not exist and a
        :raises `hasek.errors.ConnectionLock`: if the corresponding connection is currently locked
            default is not set.
        """
        if dsn is None:
    ...
    def lock_connection(cls, conf, dsn, key=None):
        ...
        :raises `hasek.errors.ConfigurationError`: if the connection does not exist
        """
    ...
    def set_value(self, key, value):
        ...
        *Note*: In order to write changes to the file, ensure that
        :meth:`~hasek.config.Config.write` is called prior to exit.
    ...
    def unlock_connection(cls, conf, dsn, key=None):
        ...
        with Config(conf, "w", key) as c:
            ...
            if connection.get("lock", None) is None:
                raise HasekError("Connection '{}' is not locked.".format(dsn))
    ...
    def unset_value(self, key):
        ...
        *Note*: In order to write changes to the file, ensure that
        ...
        :param str key: A path to the value destination, with nested levels joined by '.'
        :raises `hasek.errors.ConfigurationError`: if the key specifies an invalid path, or does not exist
        """
        if key.endswith("."):
            key = key[:-1]
        path = key.split(".")
    ...
    def write(self, settings=None):
        ...
        permissions, or if the :class:`~hasek.config.Config` object has the :code:`mode`
        'r'.
    ...
    def write_default(self, conf=None):
        ...
        :param str conf: The name of the file to write to. Defaults to :code:`None`, for ~/.hasekrc
